...
#             print(fbeta_score(labels,outputs_topN_Nhots, beta=2 ,average='samples'))
   ...
   #         break
    ...
    print('epoch' + str(epoch) + ', top' + str(topN) + ": Val Acc : %.4f, time: %d分%d秒" % (sum(fbeta_lst)/len(fbeta_lst), elapsed_time//60, int(elapsed_time % 60)))
    writer.add_scalar('eval/val_acc(epoch'+str(epoch)+')', sum(fbeta_lst)*100/len(fbeta_lst), topN)
...
for i in range(1,50):
    vgg16.load_state_dict(torch.load('./model_weight/vgg16/model_epoch' + str(i) + '.pkl'))
    for j in range(1,20):
        extract_attribute = j
        eval(i,j, writer)
